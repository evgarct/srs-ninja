...
export function ReactBitsLightPillar({
  topColor = '#5227FF',
  bottomColor = '#FF9FFC',
  intensity = 1,
  rotationSpeed = 0.3,
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        float effectiveGlow = max(uGlowAmount, 0.006);
        color = tanh(color * effectiveGlow / widthNormalization);

        float rnd = noise(gl_FragCoord.xy);
        color -= rnd / 15.0 * uNoiseIntensity;

        vec3 finalColor = max(color * uIntensity, vec3(0.0));
