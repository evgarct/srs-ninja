...
Для этой ветки допустимо удалить старые чешские данные, чтобы новый контракт не тащил legacy-поддержку в продуктовую модель.

Следствие:

- код не обязан нормализовать старые Czech payloads обратно в новый формат через скрытые alias-правила;
- import и editor должны считать legacy Czech keys неизвестными полями и не опираться на них;
- любые новые данные Czech deck должны начинаться уже с чистой схемы.
