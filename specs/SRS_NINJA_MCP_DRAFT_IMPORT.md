...
3. Если это была последняя note в batch, пустой batch удаляется автоматически.
4. Пользователь может удалить batch целиком только если в нём не осталось `approved` notes.
5. Удаление batch удаляет все оставшиеся `draft` notes внутри него.
...
- не сохранять явные точные дубликаты;
- сохранять их только по флагу `allowDuplicates`;
- или сохранять, но возвращать предупреждения по conflict list.

Главное требование: дубликаты не должны появляться полностью “молча”.
