...
Редактирование не должно ломать текущий study flow: после save UI сразу обновляет текущую ноту и все карточки в очереди с тем же `note_id`, а для English при необходимости может сразу перегенерировать аудио.
...
Для English editor показывает отдельный путь:
