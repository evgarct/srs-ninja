...
Для idle-состояний `ExtraStudyBox` сохраняет existing extra-study behavior, но без отдельного helper text. Если deck уже завершен на сегодня, extra-study menu показывает только `+10` и `+20`.
