...
| Кейс | Поведение |
|------|-----------|
| В колоде есть raw due cards, но все они уже review-ились сегодня | На дашборде основная кнопка `Учить` не показывается повторно; пользователь видит обычное состояние без агрессивного повторного prompt-а |
| Все плановые карточки повторены (`due = 0`) | На дашборде вместо кнопки «Учить» появляется единый `ExtraStudyBox` с выбором **+10** и **+20** |
