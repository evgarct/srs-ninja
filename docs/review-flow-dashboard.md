...
Поверх dashboard и review flow теперь есть и history-oriented activity surface на `/stats`, но Home сохраняет старый компактный streak widget.
...
Главная страница также использует `getWeeklyActivityStats(timeZone)` и показывает:

- последние 7 календарных дней;
- streak, идущий назад от today;
- компактный streak widget с дневными чипами;
- отдельный от `/stats` визуальный паттерн.
...
Home сохраняет компактный 7-day streak widget:

- последние 7 календарных дней;
- круговые day chips;
- отдельный `today` ring;
- tooltip по каждому дню.

### Stats heatmap

`/stats` использует более полную heatmap-визуализацию:

- локальный shadcn-style `HeatmapCalendar` primitive;
- Monday-first weekly reading;
- более длинный historical window;
- intensity-based cells в едином visual language с dashboard;
- адаптивный window: desktop и mobile используют разные range lengths.

Это делает Home компактным daily-streak surface, а Stats более глубоким historical surface.
